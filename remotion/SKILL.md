---
name: remotion
description: 用 Remotion（以 React 渲染视频的框架）编写、修改、调试视频代码时使用本技能。覆盖 Root.tsx / Composition 项目结构、媒体标签（OffthreadVideo / Img / Gif / Audio / staticFile）、布局与时间轴（AbsoluteFill / Sequence / Series / TransitionSeries）、帧驱动动画（useCurrentFrame / interpolate / spring / useVideoConfig），以及确定性渲染约束（禁止 Math.random，改用 remotion 的 random(seed)）。当用户提到：Remotion、用 React 做视频、用代码生成视频、视频合成/渲染、加转场或字幕动画、做动画短片，或出现 MyComp / Root.tsx / durationInFrames / useCurrentFrame / durationInFrames 等符号，或正在一个 remotion 项目里改代码时使用。也适用于排查「组件在渲染时不动 / 每次渲染结果不一致 / useState 无效」这类问题——Remotion 组件必须是纯函数、帧驱动、无交互。不适用于普通交互式 React 前端开发。
---

# Remotion · 用 React 写视频

This is for a Remotion based video app that uses React to render videos.

Full remotion docs can be found here: https://www.remotion.dev/docs/. Consult these docs often if you're uncertain.

## 何时使用

- 在 Remotion 项目里新增/修改 composition、场景、字幕、转场、动画
- 把一段素材（视频/图片/GIF/音频）按时间轴拼成成片
- 排查「渲染出来不动」「每次渲染不一样」「useState / onClick 不生效」等问题
- 需要判断某个 UI 组件该按交互式 React 写还是按 Remotion 帧驱动写

**不适用**：普通交互式 React 前端（有点击、有状态、有数据请求）——那是常规 React 开发，不要套用本技能的约束。

## 核心心智模型

一条视频 = **帧序号的纯函数**。给定同样的 `frame` 和 props，必须永远渲染出同样的画面。所有"随时间变化"都要从 `useCurrentFrame()` 推导，绝不能来自 `useState`、`useEffect`、`Date.now()` 或 `Math.random()`。
...
内容改编自 [ThariqS 的 Remotion CLAUDE.md gist](https://gist.github.com/ThariqS/3d446e7c7aa9eb94f468194deb73028f)，补充了技能触发描述与使用边界。
